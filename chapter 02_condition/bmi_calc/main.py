...
print(type(age))
# print(f"당신은 내년에 {age+1}살이니다.") # -> 오류 발생
'''
input() 정수의 결과값은 언제나 str입니다. -> 즉, 수학  연산을 하기 위해서는
별도의 과정이 필요합니다.
...
2.float() -> str 또는 int을 float로 변경   ->정수의 경우 
3.
'''
temp =int(3,8)
print(temp)
...
tmep4 = round(5.3491285, 2)    #괄호 첫번쨰 수르  소수점 2쨰짜리까지 표기
'''
Bmi 계산기를 작성합니다.
...
5. bmi 지수를 소수점 셋쨰자리에서 반올림하면 둘째자리까지 출력하세요. -> round()함수 사용
...
'''
 ...
 height=float(height)
 height = height / 100 # cm=>m로 단위 변환                               #이미 float으로 바뀌었기 떄문에 height러 입력해도 됨
 weight = input("당신의 몸무게는 몇 kg입니까 >>>")
 weight = float(weight)
 bmi = weight / (height**2) #몸무게 / 키(m)의 제곱 ->결과값이 float으로 나옴
 bmi_int = int(bmi)
 bmi_round = round(bmi, 2)  #계싼 후 결과값을 소수점 셋째자리에서 반올림한 결과
 print(f"단신의 bmi지수는 {bmi_int}입니다.")

 # 가장 짧지만 가독성이 별로인 코드
 height = float(input("당신의 키는 몇 cm입니까? >>>")) / 100 #키 입력받고 바로 m로 변환
 weight = float(input("단신의 몸무게는 몇 kg 입니까? >>>"))
 print(f"단신의 bmi지수는 {int(weight/(height **2))}입니다.")
 print(f"당신의  bmi지수는 {round(weight/height**2)}입니다.")

  #지시사항 3번과 같이 연산을 해야함 -> 연산전에 height울 cm -> m표기로 바꿀 필요가 있음
  # print(type(height)) #str으로 나오기 떄문에 수학 연산이 불가능 합니다.
  # height / weight 가 str 이기 때문에 수학 연산이 가능한 자료형 으로 형 변환을 할 필요가 있음.
