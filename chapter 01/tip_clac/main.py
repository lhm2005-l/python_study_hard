    ...
    #percent = float(input("몇 퍼센트(%)의 팁을 내실 예정입니까,10, 12, 15 중에서"))
    #people = int(input("먗 몇의 인원이 나누어 내나요? >>>"))

    # percent가 10, 12, 15중에 하나니깐 예를 0.1, 0.12, 0.15로 바꿀 필요 있음.
    # food의 percent를 가지고 아떻게 잘 연산을 해서 전체 음식값(음식값+팁)을 산출.
    # 전체 음식값에서 people러 나누어야 합니다.
    # 맨 마지막에 출력문을 f-string으로 작성해야 합니다.

    # percent를 소수점으로 바꾸는 연산
    #percent = percent / 100
    #tip = food 8 percent           # 팁 가격만 산출
    #total_food = food + tip         # 전체 음식 가격 산출
    ...
    # 최대한 짧은 버전으로 작성하자면
    food = float(input("음식 가격은 얼마입니까? >>> $"))
    percent = float(input("몇 퍼센트(%)의 팁을 내실 예정입니까, 10, 12, 15중에서 선택하세요. >>>")) / 100
    ...
    # 35번 라인의 경우는 round() 함수와 달라 어떤 상황에서도 소수점 둘째자리까지 표기되도록 강제하는 코드